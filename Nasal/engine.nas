...
var engineControl = func {
	...
	var rpm = getprop("/engines/engine[0]/rpm");
	if (getprop("/accelerations/pilot-gdamped") >= 0) var g_sign = 1;
	else var g_sign = -1;
	...
	if (oil_temp > max_oil_temp) {
		var rpm_loss = 1 - (oil_temp - max_oil_temp) * 0.1;
		if (rpm_loss < 0) rpm_loss = 0;
		setprop("/fdm/jsbsim/propulsion/engine/rpm-loss", rpm_loss);
		setprop("/controls/engines/engine[0]/faults/smoke", 1 - rpm_loss);
	} elsif (getprop("/fdm/jsbsim/propulsion/engine/rpm-loss") != 1 or getprop("/controls/engines/engine[0]/faults/smoke") != 0) {
		var rpm_loss = 1;
		setprop("/fdm/jsbsim/propulsion/engine/rpm-loss", rpm_loss);
	...
	if (g_sign < 0) {
		...
		setprop("/fdm/jsbsim/propulsion/engine/rpm-loss", 1);
	}
	if (neg_g_fuel_factor > max_neg_g_fuel_factor) {
		setprop("/fdm/jsbsim/propulsion/engine/rpm-loss", 0);
	}
